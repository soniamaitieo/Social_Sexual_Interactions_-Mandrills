...
setwd("/home/sonia/Perception_Typicality_CNN/Paper1/Figures/forestplot_month_2_dcf")
...
dfall$Mean_dcF=scale(dfall$Mean_dcF)
...
applySPAMM <- function(formula,df){
  ...
  CycleT$CycleBin <- "yes"
  pdep_F <- pdep_effects(spfitMO,"Mean_dcF", newdata=as.data.frame(CycleF), length.out = 500)
  pdep_T <- pdep_effects(spfitMO,"Mean_dcF", newdata=as.data.frame(CycleT), length.out = 500)
  pdep_F$CycleBin <- "no"
  pdep_T$CycleBin <- "yes"
...
applySPAMM_onevar <- function(formula,df){
  ...
  forrates$Tps_Foc_IndAllmal <- 1L
  forrates$Nb_Scan_IndAllmal <- 1L
  pdep <- pdep_effects(spfitMO,"Mean_dcF", newdata=as.data.frame(forrates), length.out = 500)
  return(list(mymodel=spfitMO, pdep=pdep))
}
...
applySPAMM_2var <- function(formula,df){
  ...
  pdep_NO <- pdep_effects(spfitMO,"Mean_dcF", newdata=as.data.frame(CycleNO), length.out = 500)
  pdep <- pdep_effects(spfitMO,"Mean_dcF", newdata=as.data.frame(Cycle), length.out = 500)
...
# MO1
MOformula = "MO_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
...
MOformula = "MO_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg + saison_repro + saison_repro*Mean_dcF +offset(log(Tps_Foc_IndAllmal))"
...
MOformula = "MO_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg + saison_repro + CycleCateg*Mean_dcF +offset(log(Tps_Foc_IndAllmal))"
spfitMO = applySPAMM_2var(MOformula,dfall)
summary(spfitMO$mymodel,details=c(p_value="Wald"))
plot_model(spfitMO$mymodel, show.values = TRUE, vline.color = "lightgrey", value.offset = .3, axis.title = "Estimates", title = "") + 
  theme(face = "bold") + theme_light(base_size=16)
...
# TO1
TOformula = "TO_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg +  saison_repro +offset(log(Tps_Foc_IndAllmal))"
...
# AG1
AGformula = "AGall_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg  + saison_repro +offset(log(Tps_Foc_IndAllmal))"
...
#AG2
AGformula = "AGmild_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg + offset(log(Tps_Foc_IndAllmal))"
...
#AG2
AGformula = "AGsev_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
    rang + CycleCateg + offset(log(Tps_Foc_IndAllmal))"
...
# SCAN1
SCANformula = "SCAN_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
...
AGbySCANformula = "AGall_Nb ~ (1 | id) + (1 | year) + Mean_dcF + age +  
